...
var auth = 'Basic xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx';
var headers = {'Authorization': auth, accept: 'application/json'};

var updateDockerCloud = function(resource_records, next) {
  
  async.waterfall([
    function listNodes(next) {
      console.log("Pinging dockercloud for nodes");
      var objs = []
      var req = https.request({
        host: "cloud.docker.com",
        path: "/api/infra/v1/node/",
        method: "GET",
        headers: headers,
      }, function(res) {
        ...
        res.on('data', function(chunk) {
          ...
          if (chunk.objects) {
            objs = objs.concat(chunk.objects);
          }
        });
        res.on('end', function() {
    ...
    function terminateNodes(objs, next) {
      var fns = objs.map(function(o) {
       return function(next){
        var req = https.request({
          ...
          res.setEncoding('utf8');
          res.on('end', function() {
            console.log("terminated " + o.uuid + ", was in state" + o.state);
            next();
          });
        });
    ...
    if (err) {
      console.log("Failed to update docker cloud nodes: ");
      console.log(err);
    } else {
      console.log("Successfully updated docker cloud nodes.");
    }
    next(err);
  });
};
  ...
  if (asg_event === "autoscaling:EC2_INSTANCE_LAUNCH" || asg_event === "autoscaling:EC2_INSTANCE_TERMINATE") {
    ...
    async.waterfall([
      ...
      function updateDNS(route53Tags, ec2Response, next) {
        ...
        function(next) {
          updateDockerCloud(resource_records, next);
        }], next);
      }
    ], function (err) {
      if (err) {
